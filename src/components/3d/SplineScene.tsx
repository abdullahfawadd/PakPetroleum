...
function NetworkGraph() {
  ...
  const connections = useMemo(() => {
    ...
    // OPTIMIZATION: Use for-loops with i+1 to prevent redundant checks,
    // and distanceToSquared to avoid expensive Math.sqrt calls.
    const thresholdSquared = 2.5 * 2.5;
    for (let i = 0; i < nodes.length; i++) {
      for (let j = i + 1; j < nodes.length; j++) {
        const node = nodes[i];
        const other = nodes[j];
        const distSq = node.distanceToSquared(other);
