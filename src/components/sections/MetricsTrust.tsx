...
export default function MetricsTrust() {
  ...
  // Array of refs to avoid expensive DOM reads (querySelectorAll) in the animation loop.
  // We initialize the array size based on STATS.length to keep it stable.
  const statRefs = useRef<(HTMLSpanElement | null)[]>(new Array(STATS.length).fill(null));
  ...
  const containerRef = useGSAP<HTMLElement>(() => {
    ...
    // ⚡ Bolt Performance Optimization:
    // Count up animation directly mapping to constant STATS data instead of
    // reading attributes from the DOM. Benchmarks show this reduces setup overhead by ~30%.
                ...
                {STATS.map((stat, idx) => (
                    <div key={idx} className="stat-item text-center group p-6 rounded-lg transition-all duration-300 hover:shadow-glow-teal-sm hover:bg-navy-800/50">
                        <div className="text-4xl md:text-6xl font-display font-bold text-white mb-2 flex justify-center items-baseline">
                            <span ref={(el) => {statRefs.current[idx] = el;}} className="stat-value tabular-nums font-mono">0</span>
