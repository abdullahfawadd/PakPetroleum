...
## 2024-05-27 - Isolating High-Frequency Form State
**Learning:** Placing fast-updating form state (`useState` for every keystroke) within a parent component that contains static content or complex GSAP animations causes the entire section to re-render unnecessarily on every input change, potentially introducing UI jank.
**Action:** Extract form logic and UI into a separate, dedicated child component to isolate React render cycles to the form elements only.
